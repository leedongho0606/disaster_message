...
class WebhookWoker {
    ...
    Send(content) {
        if (!this.CanSend) return Log("전송차단됨");
        Log("총", this.webhookClients.length, "개의 등록된 웹훅으로 메시지 전송중...");
    ...
    async _webhookRegister(webhooks) {
        for (let url of webhooks) {
            ...
            Log("전송할 웹훅목록에 등록", addmsg ? addmsg : "성공");
            if (addmsg) continue;
            this.webhookClients.push(currentUrl);
        }
        Log("전송할 웹훅목록 등록작업 완료. 전체", this.webhookClients.length, "개");
    }
